    ...
    <div class="sub-articlespost">
      <div class="content-sub-articlespost">
        <div class="content-sub-articlespost___title">
          {{this.$store.getters['posts/filterPosts']('Космос')[this.$route.params.id - 1].title}}
        </div>
        <div class="content-sub-articlespost___body">
          <div class="content-sub-articlespost___element">
            {{this.$store.getters['posts/filterPosts']('Космос')[this.$route.params.id - 1].body}}
            <img style="width: 100%; margin: 15px 15px 0 0;"
                 :src="this.$store.getters['posts/filterPosts']('Космос')[this.$route.params.id - 1].img"
                 alt="image">
          </div>
        </div>
      </div>
      ...
      <div class="content-sub-articlespost">
        ...
        <div class="content-sub-articlespost___body">
          <div class="content-sub-articlespost___block"
               v-for="(comment, index) in this.$store.getters['comments/getComments'](this.$store.getters['posts/filterPosts']('Космос')[this.$route.params.id - 1].id)"
               :key="comment">
...
export default {
  created () {
    console.log(this.$store.getters['posts/filterPosts']('Космос'))
  },
  data () {
    return {
      body: '',
    ...
    addComment () {
      if (this.$store.getters['users/isAuth']) {
        this.$store.dispatch('comments/createComment', {
          body: this.body,
          idPost: this.$store.getters['posts/filterPosts']('Космос')[this.$route.params.id - 1].id,
